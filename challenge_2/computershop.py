inventory = {
    ...
    "ram": {"B1":["8GB RAM", 79.99], "B2":["16GB RAM", 149.99], "B3":["32GB RAM", 299.99]},
    "mainHDD": {"C1":["1 TB HDD", 49.99], "C2": ["2 TB HDD", 89.99], "C3": ["4 TB HDD", 129.99]},
    "SSD": {"D1": ["240GB", 59.99], "D2": ["480GB", 119.99]},
    "secondaryHDD": {"E1": ["1 TB", 49.99], "E2": ["2 TB", 89.99], "E3": ["4 TB", 129.99]},
    "opticalDrive": {"F1": ["DVD/Blu-Ray Player", 50.00], "F2": ["DVD/Blu-Ray Re-Writer", 100.00]},
    "operatingSystem": {"G1": ["Standard Edition", 100.00], "G2": ["Professional Edition", 175.00]},
}
width = 50
def listItems(query, title):
    ...
    for itemCode in inventory[query]:
            space = len(str(inventory[query][itemCode][0])) + len(str(inventory[query][itemCode][1]))
            rem = width - space - 6
...
def itemsTable(itemList, heading):
    ...
    for item in itemList:
        ...
        print(f"│{' '*((1+width-len(itemName)-len(itemCost)-4)//2)}{itemName}:  {itemCost}${' '*((width - len(itemName) -len(itemCost)-4)//2)}│")
    print(f"╰{'─'*(width)}╯\n╭{'─'*width}╮")
...
def main():
    ...
    if addlItems == 'yes':
        ...
        while addlRequest == True:
            ...
            itemDescription = ["Case", "RAM", "Main Hard Drive", "Secondary Hard Drive", "Solid State Drive", "Optical Drive", "Operating System"]
            print(f"╭{'─'*width}╮\n│{heading}{' '*(width-len(heading))}│\n├{'─'*width}┤")
            i = 1
